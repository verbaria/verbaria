...
import org.verbaria.server.ui.vaadin.ai.TranslationUIProvider;
...
public class AdminAiSettingsView extends VerticalLayout implements TitleKey {
    ...
    private final Map<String, TranslationUIProvider> uiProvidersById;

    public AdminAiSettingsView(TranslationProviderRegistry registry,
                               AiSettingsService settings,
                               AiPolicyService policy,
                               List<TranslationUIProvider> uiProviders) {
        this.uiProvidersById = uiProviders.stream().collect(Collectors.toMap(
                TranslationUIProvider::providerId, Function.identity(),
                (a, b) -> a));
        setWidthFull();
        setHeightFull();
        setPadding(true);
    ...
    private Div buildProviderCard(
        ...
        TranslationUIProvider ui = uiProvidersById.get(p.id());
        if (ui != null) {
            card.add(ui.createSettingsPage());
        }
        return card;
    }
}
